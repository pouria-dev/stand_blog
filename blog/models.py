...
class Category(models.Model):
    ...
    )  # slug field for SEO and clean url
...
class Article(models.Model):
    ...
    banner = models.ImageField(
        upload_to="article/banner", help_text="best size for banner:770x340"
    )
    status = models.BooleanField(default=False)
    slug = models.SlugField(null=True, unique=True, blank=True)  # slug field for SEO
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
    objects = Model_Manager()  # Custom manager for additional functionality
